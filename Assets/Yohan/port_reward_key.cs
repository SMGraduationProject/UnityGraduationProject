...
public class port_reward_key : port_reward_chest      // 상속할 스크립트는 castle_puzzle_chest의 이름을 변경하여 넣으시면 됩니다.
{
    ...
    Vector3 port_key_pos;             // 키의 초기 위치를 위한 변수
    Vector3 port_key_cur_pos;         // 키의 실시간 위치를 위한 변수
    ...
    {
        port_key_cur_pos = this.transform.position;       // 키의 실시간 위치를 업데이트 시키는 코드

        if (port_key_cur_pos.y < port_key_pos.y + 1)        // 키의 실시간 위치가 초기 위치에 비해 얼마나 높은가를 검사하는 조건문. 방향을 바꾸려면 y를 다른것으로 바꾸면 됩니다.
        {
    ...
    void Start()
    {
        port_key_pos = this.transform.position;           // 키의 초기 위치를 적용시키는 코드.
    }

    // Update is called once per frame
    void Update()
    {
        if (port_key == 1)                // 시작하려면 castle_key부분을 여러분들이 변경시킨 전역변수명을 대신 쓰시면 됩니다.
        {
