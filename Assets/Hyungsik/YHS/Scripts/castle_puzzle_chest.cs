...
public class castle_puzzle_chest : castle_puzzle              // 상속할 스크립트는 castle_puzzle 대신 여러분들의 퍼즐 스크립트를 넣으시면 되며 그 스크립트 안에 이 스크립트를 작동시킬 전역변수를 하나 선언하시면 됩니다.
{
    ...
    void Update()
    {
        if (castle_chest == 1)      // 전역변수에 1을 추가하여 조건식을 이용해 애니메이션을 작동시킵니다. 전역변수명은 여러분이 지정한 변수명을 사용하면 됩니다.
        {
            if(opened == 0)         // 박스가 열렸는지를 확인하기 위한 조건문
            {
                ...
                castle_key = 1;     // 전역변수에 1을 추가하여 key 스크립트를 작동시킵니다. 전역변수명은 여러분이 지정한 변수명을 사용하면 됩니다.
            }
        }
    }
}
